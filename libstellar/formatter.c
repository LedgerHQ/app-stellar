...
static bool format_transaction_source(formatter_data_t *fdata) {
    ...
    if (fdata->envelope->type == ENVELOPE_TYPE_TX &&
        fdata->envelope->tx.source_account.type == KEY_TYPE_ED25519 &&
        memcmp(fdata->envelope->tx.source_account.ed25519,
               fdata->signing_key,
               RAW_ED25519_PUBLIC_KEY_SIZE) == 0) {
        FORMATTER_CHECK(print_muxed_account(&fdata->envelope->tx.source_account,
                                            fdata->value,
                                            fdata->value_len,
                                            6,
                                            6))
...
static bool format_min_seq_ledger_gap(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_uint(fdata->envelope->tx.cond.min_seq_ledger_gap, fdata->value, fdata->value_len))
...
static bool format_min_seq_ledger_gap_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.cond.min_seq_ledger_gap == 0) {
...
static bool format_min_seq_age(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_uint(fdata->envelope->tx.cond.min_seq_age, fdata->value, fdata->value_len))
...
static bool format_min_seq_age_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.cond.min_seq_age == 0) {
...
static bool format_min_seq_num(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_uint(fdata->envelope->tx.cond.min_seq_num, fdata->value, fdata->value_len))
...
static bool format_min_seq_num_prepare(formatter_data_t *fdata) {
    if (!fdata->envelope->tx.cond.min_seq_num_present ||
        fdata->envelope->tx.cond.min_seq_num == 0) {
...
static bool format_ledger_bounds_max_ledger(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.cond.ledger_bounds.max_ledger,
                               fdata->value,
                               fdata->value_len))
...
static bool format_ledger_bounds_min_ledger(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.cond.ledger_bounds.min_ledger,
                               fdata->value,
                               fdata->value_len))
    if (fdata->envelope->tx.cond.ledger_bounds.max_ledger != 0) {
...
static bool format_ledger_bounds(formatter_data_t *fdata) {
    if (!fdata->envelope->tx.cond.ledger_bounds_present ||
        (fdata->envelope->tx.cond.ledger_bounds.min_ledger == 0 &&
         fdata->envelope->tx.cond.ledger_bounds.max_ledger == 0)) {
        return format_min_seq_num_prepare(fdata);
    } else if (fdata->envelope->tx.cond.ledger_bounds.min_ledger != 0) {
...
static bool format_time_bounds_max_time(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_time(fdata->envelope->tx.cond.time_bounds.max_time, fdata->value, fdata->value_len))
...
static bool format_time_bounds_min_time(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_time(fdata->envelope->tx.cond.time_bounds.min_time, fdata->value, fdata->value_len))

    if (fdata->envelope->tx.cond.time_bounds.max_time != 0) {
...
static bool format_time_bounds(formatter_data_t *fdata) {
    if (!fdata->envelope->tx.cond.time_bounds_present ||
        (fdata->envelope->tx.cond.time_bounds.min_time == 0 &&
         fdata->envelope->tx.cond.time_bounds.max_time == 0)) {
        return format_ledger_bounds(fdata);
    } else if (fdata->envelope->tx.cond.time_bounds.min_time != 0) {
...
static bool format_sequence(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.sequence_number, fdata->value, fdata->value_len))
...
static bool format_fee(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.fee,
                                 &asset,
...
static bool format_memo(formatter_data_t *fdata) {
    memo_t *memo = &fdata->envelope->tx.memo;
    switch (memo->type) {
        case MEMO_ID: {
...
static bool format_transaction_details(formatter_data_t *fdata) {
    ...
    if (fdata->envelope->tx.memo.type != MEMO_NONE) {
...
static bool format_operation_source(formatter_data_t *fdata) {
    ...
    if (fdata->envelope->type == ENVELOPE_TYPE_TX &&
        fdata->envelope->tx.source_account.type == KEY_TYPE_ED25519 &&
        fdata->envelope->tx.op_details.source_account.type == KEY_TYPE_ED25519 &&
        memcmp(fdata->envelope->tx.source_account.ed25519,
               fdata->signing_key,
               RAW_ED25519_PUBLIC_KEY_SIZE) == 0 &&
        memcmp(fdata->envelope->tx.op_details.source_account.ed25519,
               fdata->signing_key,
               RAW_ED25519_PUBLIC_KEY_SIZE) == 0) {
        FORMATTER_CHECK(print_muxed_account(&fdata->envelope->tx.op_details.source_account,
                                            fdata->value,
                                            fdata->value_len,
                                            6,
                                            6))
...
static bool format_operation_source_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.source_account_present) {
...
static bool format_bump_sequence_bump_to(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_int(fdata->envelope->tx.op_details.bump_sequence_op.bump_to,
                              fdata->value,
                              fdata->value_len))
...
static bool format_account_merge_destination(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_muxed_account(&fdata->envelope->tx.op_details.account_merge_op.destination,
                            fdata->value,
                            fdata->value_len,
                            0,
                            0))
...
static bool format_manage_data_value(formatter_data_t *fdata) {
    // TODO: improve
    char tmp[89];
    if (is_printable_binary(fdata->envelope->tx.op_details.manage_data_op.data_value,
                            fdata->envelope->tx.op_details.manage_data_op.data_value_size)) {
        STRLCPY(fdata->caption, "Data Value", fdata->caption_len);
        memcpy(tmp,
               (char *) fdata->envelope->tx.op_details.manage_data_op.data_value,
               fdata->envelope->tx.op_details.manage_data_op.data_value_size);
        tmp[fdata->envelope->tx.op_details.manage_data_op.data_value_size] = '\0';
        STRLCPY(fdata->value, tmp, fdata->value_len);
    } else {
        STRLCPY(fdata->caption, "Data Value (base64)", fdata->caption_len);
        FORMATTER_CHECK(base64_encode(fdata->envelope->tx.op_details.manage_data_op.data_value,
                                      fdata->envelope->tx.op_details.manage_data_op.data_value_size,
                                      tmp,
                                      sizeof(tmp)))
        FORMATTER_CHECK(print_summary(tmp, fdata->value, fdata->value_len, 6, 6))
...
static bool format_manage_data(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.manage_data_op.data_value_size) {
    ...
    char tmp[65];
    memcpy(tmp,
           fdata->envelope->tx.op_details.manage_data_op.data_name,
           fdata->envelope->tx.op_details.manage_data_op.data_name_size);
    tmp[fdata->envelope->tx.op_details.manage_data_op.data_name_size] = '\0';
    STRLCPY(fdata->value, tmp, fdata->value_len);
    return true;
}
...
static bool format_allow_trust_authorize(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_allow_trust_flags(fdata->envelope->tx.op_details.allow_trust_op.authorize,
                                            fdata->value,
                                            fdata->value_len))
    format_operation_source_prepare(fdata);
    return true;
...
static bool format_allow_trust_asset_code(formatter_data_t *fdata) {
    ...
    STRLCPY(fdata->value,
            fdata->envelope->tx.op_details.allow_trust_op.asset_code,
            fdata->value_len);
...
static bool format_allow_trust_trustor(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_account_id(fdata->envelope->tx.op_details.allow_trust_op.trustor,
                                     fdata->value,
                                     fdata->value_len,
                                     0,
                                     0))
...
static bool format_set_option_signer_weight(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.op_details.set_options_op.signer.weight,
                               fdata->value,
                               fdata->value_len))
    format_operation_source_prepare(fdata);
    return true;
...
static bool format_set_option_signer_detail(formatter_data_t *fdata) {
    ...
    signer_key_t *key = &fdata->envelope->tx.op_details.set_options_op.signer.key;
    ...
    if (fdata->envelope->tx.op_details.set_options_op.signer.weight != 0) {
...
static bool format_set_option_signer(formatter_data_t *fdata) {
    signer_t *signer = &fdata->envelope->tx.op_details.set_options_op.signer;
    if (signer->weight) {
...
static bool format_set_option_signer_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.signer_present) {
...
static bool format_set_option_home_domain(formatter_data_t *fdata) {
    ...
    if (fdata->envelope->tx.op_details.set_options_op.home_domain_size) {
        memcpy(fdata->value,
               fdata->envelope->tx.op_details.set_options_op.home_domain,
               fdata->envelope->tx.op_details.set_options_op.home_domain_size);
        fdata->value[fdata->envelope->tx.op_details.set_options_op.home_domain_size] = '\0';
    } else {
...
static bool format_set_option_home_domain_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.home_domain_present) {
...
static bool format_set_option_high_threshold(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.op_details.set_options_op.high_threshold,
                               fdata->value,
                               fdata->value_len))
...
static bool format_set_option_high_threshold_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.high_threshold_present) {
...
static bool format_set_option_medium_threshold(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.op_details.set_options_op.medium_threshold,
                               fdata->value,
                               fdata->value_len))
...
static bool format_set_option_medium_threshold_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.medium_threshold_present) {
...
static bool format_set_option_low_threshold(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.op_details.set_options_op.low_threshold,
                               fdata->value,
                               fdata->value_len))
...
static bool format_set_option_low_threshold_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.low_threshold_present) {
...
static bool format_set_option_master_weight(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_uint(fdata->envelope->tx.op_details.set_options_op.master_weight,
                               fdata->value,
                               fdata->value_len))
...
static bool format_set_option_master_weight_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.master_weight_present) {
...
static bool format_set_option_set_flags(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_account_flags(fdata->envelope->tx.op_details.set_options_op.set_flags,
                                        fdata->value,
                                        fdata->value_len))
...
static bool format_set_option_set_flags_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.set_flags_present) {
...
static bool format_set_option_clear_flags(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_account_flags(fdata->envelope->tx.op_details.set_options_op.clear_flags,
                                        fdata->value,
                                        fdata->value_len))
...
static bool format_set_option_clear_flags_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.clear_flags_present) {
...
static bool format_set_option_inflation_destination(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_account_id(fdata->envelope->tx.op_details.set_options_op.inflation_destination,
                         fdata->value,
                         fdata->value_len,
                         0,
                         0))
...
static bool format_set_option_inflation_destination_prepare(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.set_options_op.inflation_destination_present) {
...
static bool is_empty_set_options_body(formatter_data_t *fdata) {
    return !(fdata->envelope->tx.op_details.set_options_op.inflation_destination_present ||
             fdata->envelope->tx.op_details.set_options_op.clear_flags_present ||
             fdata->envelope->tx.op_details.set_options_op.set_flags_present ||
             fdata->envelope->tx.op_details.set_options_op.master_weight_present ||
             fdata->envelope->tx.op_details.set_options_op.low_threshold_present ||
             fdata->envelope->tx.op_details.set_options_op.medium_threshold_present ||
             fdata->envelope->tx.op_details.set_options_op.high_threshold_present ||
             fdata->envelope->tx.op_details.set_options_op.home_domain_present ||
             fdata->envelope->tx.op_details.set_options_op.signer_present);
}
...
static bool format_change_trust_limit(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.op_details.change_trust_op.limit,
                                 NULL,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_change_trust_detail_liquidity_pool_fee(formatter_data_t *fdata) {
    ...
    uint64_t fee = ((uint64_t) fdata->envelope->tx.op_details.change_trust_op.line.liquidity_pool
    ...
    if (fdata->envelope->tx.op_details.change_trust_op.limit &&
        fdata->envelope->tx.op_details.change_trust_op.limit != INT64_MAX) {
...
static bool format_change_trust_detail_liquidity_pool_asset_b(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_asset(&fdata->envelope->tx.op_details.change_trust_op.line.liquidity_pool
                                     .constant_product.asset_b,
                                fdata->envelope->network,
                                fdata->value,
                                fdata->value_len))
...
static bool format_change_trust(formatter_data_t *fdata) {
    ...
    uint8_t asset_type = fdata->envelope->tx.op_details.change_trust_op.line.type;
    switch (asset_type) {
        case ASSET_TYPE_CREDIT_ALPHANUM4:
        case ASSET_TYPE_CREDIT_ALPHANUM12:
            FORMATTER_CHECK(
                print_asset((asset_t *) &fdata->envelope->tx.op_details.change_trust_op.line,
                            fdata->envelope->network,
                            fdata->value,
                            fdata->value_len))
            if (fdata->envelope->tx.op_details.change_trust_op.limit &&
...
static bool format_manage_sell_offer_price(formatter_data_t *fdata) {
    ...
    uint64_t price =
        ((uint64_t) fdata->envelope->tx.op_details.manage_sell_offer_op.price.n * 10000000) /
        fdata->envelope->tx.op_details.manage_sell_offer_op.price.d;
    FORMATTER_CHECK(
        print_amount(price, NULL, fdata->envelope->network, fdata->value, fdata->value_len))
    STRLCAT(fdata->value, " ", fdata->value_len);
    char tmp_asset_code[13] = {0};
    FORMATTER_CHECK(print_asset_name(&fdata->envelope->tx.op_details.manage_sell_offer_op.buying,
                                     fdata->envelope->network,
                                     tmp_asset_code,
                                     sizeof(tmp_asset_code)))
    STRLCAT(fdata->value, tmp_asset_code, fdata->value_len);
    STRLCAT(fdata->value, "/", fdata->value_len);
    FORMATTER_CHECK(print_asset_name(&fdata->envelope->tx.op_details.manage_sell_offer_op.selling,
                                     fdata->envelope->network,
                                     tmp_asset_code,
                                     sizeof(tmp_asset_code)))
    STRLCAT(fdata->value, tmp_asset_code, fdata->value_len);
...
static bool format_manage_sell_offer_sell(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.op_details.manage_sell_offer_op.amount,
                                 &fdata->envelope->tx.op_details.manage_sell_offer_op.selling,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_manage_sell_offer_buy(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_asset(&fdata->envelope->tx.op_details.manage_sell_offer_op.buying,
                                fdata->envelope->network,
                                fdata->value,
                                fdata->value_len))
...
static bool format_manage_sell_offer(formatter_data_t *fdata) {
    if (!fdata->envelope->tx.op_details.manage_sell_offer_op.amount) {
        STRLCPY(fdata->caption, "Remove Offer", fdata->caption_len);
        FORMATTER_CHECK(print_uint(fdata->envelope->tx.op_details.manage_sell_offer_op.offer_id,
                                   fdata->value,
                                   fdata->value_len))
        format_operation_source_prepare(fdata);
    } else {
        if (fdata->envelope->tx.op_details.manage_sell_offer_op.offer_id) {
...
static bool format_manage_buy_offer_price(formatter_data_t *fdata) {
    manage_buy_offer_op_t *op = &fdata->envelope->tx.op_details.manage_buy_offer_op;
    ...
    STRLCAT(fdata->value, " ", fdata->value_len);
    char tmp_asset_code[13] = {0};
    FORMATTER_CHECK(print_asset_name(&fdata->envelope->tx.op_details.manage_buy_offer_op.selling,
                                     fdata->envelope->network,
                                     tmp_asset_code,
                                     sizeof(tmp_asset_code)))
    STRLCAT(fdata->value, tmp_asset_code, fdata->value_len);
    STRLCAT(fdata->value, "/", fdata->value_len);
    FORMATTER_CHECK(print_asset_name(&fdata->envelope->tx.op_details.manage_buy_offer_op.buying,
                                     fdata->envelope->network,
                                     tmp_asset_code,
                                     sizeof(tmp_asset_code)))
    STRLCAT(fdata->value, tmp_asset_code, fdata->value_len);
...
static bool format_create_passive_sell_offer_price(formatter_data_t *fdata) {
    ...
    char tmp_asset_code[13] = {0};
    FORMATTER_CHECK(
        print_asset_name(&fdata->envelope->tx.op_details.create_passive_sell_offer_op.buying,
                         fdata->envelope->network,
                         tmp_asset_code,
                         sizeof(tmp_asset_code)))
    STRLCAT(fdata->value, tmp_asset_code, fdata->value_len);
    STRLCAT(fdata->value, "/", fdata->value_len);
    FORMATTER_CHECK(
        print_asset_name(&fdata->envelope->tx.op_details.create_passive_sell_offer_op.selling,
                         fdata->envelope->network,
                         tmp_asset_code,
                         sizeof(tmp_asset_code)))
    STRLCAT(fdata->value, tmp_asset_code, fdata->value_len);
...
static bool format_create_passive_sell_offer_sell(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Sell", fdata->caption_len);
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.create_passive_sell_offer_op.amount,
                     &fdata->envelope->tx.op_details.create_passive_sell_offer_op.selling,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
...
static bool format_create_passive_sell_offer_buy(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Buy", fdata->caption_len);
    FORMATTER_CHECK(print_asset(&fdata->envelope->tx.op_details.create_passive_sell_offer_op.buying,
                                fdata->envelope->network,
                                fdata->value,
                                fdata->value_len))
...
static bool format_path_payment_strict_receive_receive(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.path_payment_strict_receive_op.dest_amount,
                     &fdata->envelope->tx.op_details.path_payment_strict_receive_op.dest_asset,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
    format_operation_source_prepare(fdata);
...
static bool format_path_payment_strict_receive_destination(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_muxed_account(
        &fdata->envelope->tx.op_details.path_payment_strict_receive_op.destination,
        fdata->value,
        fdata->value_len,
        0,
        0))
...
static bool format_path_payment_strict_receive(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.path_payment_strict_receive_op.send_max,
                     &fdata->envelope->tx.op_details.path_payment_strict_receive_op.send_asset,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
...
static bool format_path_payment_strict_send_receive(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.path_payment_strict_send_op.dest_min,
                     &fdata->envelope->tx.op_details.path_payment_strict_send_op.dest_asset,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
    format_operation_source_prepare(fdata);
...
static bool format_path_payment_strict_send_destination(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Destination", fdata->caption_len);
    FORMATTER_CHECK(
        print_muxed_account(&fdata->envelope->tx.op_details.path_payment_strict_send_op.destination,
                            fdata->value,
                            fdata->value_len,
                            0,
                            0))
...
static bool format_path_payment_strict_send(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.path_payment_strict_send_op.send_amount,
                     &fdata->envelope->tx.op_details.path_payment_strict_send_op.send_asset,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
...
static bool format_payment_destination(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Destination", fdata->caption_len);
    FORMATTER_CHECK(print_muxed_account(&fdata->envelope->tx.op_details.payment_op.destination,
                                        fdata->value,
                                        fdata->value_len,
                                        0,
                                        0))
...
static bool format_payment(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Send", fdata->caption_len);
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.op_details.payment_op.amount,
                                 &fdata->envelope->tx.op_details.payment_op.asset,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_create_account_amount(formatter_data_t *fdata) {
    ...
    asset_t asset = {.type = ASSET_TYPE_NATIVE};
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.op_details.create_account_op.starting_balance,
                                 &asset,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_create_account_destination(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Destination", fdata->caption_len);
    FORMATTER_CHECK(print_account_id(fdata->envelope->tx.op_details.create_account_op.destination,
                                     fdata->value,
                                     fdata->value_len,
                                     0,
                                     0))
...
static bool format_create_claimable_balance_balance(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.op_details.create_claimable_balance_op.amount,
                                 &fdata->envelope->tx.op_details.create_claimable_balance_op.asset,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_claim_claimable_balance_balance_id(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_claimable_balance_id(
        &fdata->envelope->tx.op_details.claim_claimable_balance_op.balance_id,
        fdata->value,
        fdata->value_len,
        12,
        12))
...
static bool format_claim_claimable_balance_sponsored_id(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_account_id(
        fdata->envelope->tx.op_details.begin_sponsoring_future_reserves_op.sponsored_id,
        fdata->value,
        fdata->value_len,
        0,
        0))
...
static bool format_revoke_sponsorship_account(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_account_id(
        fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.account.account_id,
        fdata->value,
        fdata->value_len,
        0,
        0))
...
static bool format_revoke_sponsorship_trust_line_asset(formatter_data_t *fdata) {
    if (fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.trust_line.asset.type ==
        ASSET_TYPE_POOL_SHARE) {
        STRLCPY(fdata->caption, "Liquidity Pool ID", fdata->caption_len);
        FORMATTER_CHECK(print_binary(fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key
        ...
        FORMATTER_CHECK(print_asset((asset_t *) &fdata->envelope->tx.op_details
...
static bool format_revoke_sponsorship_trust_line_account(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Account ID", fdata->caption_len);
    FORMATTER_CHECK(print_account_id(
        fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.trust_line.account_id,
        fdata->value,
        fdata->value_len,
        0,
        0))
...
static bool format_revoke_sponsorship_offer_offer_id(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_uint(fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.offer.offer_id,
                   fdata->value,
                   fdata->value_len))
...
static bool format_revoke_sponsorship_offer_seller_id(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_account_id(
        fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.offer.seller_id,
        fdata->value,
        fdata->value_len,
        0,
        0))
...
static bool format_revoke_sponsorship_data_data_name(formatter_data_t *fdata) {
    ...
    memcpy(fdata->value,
           fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.data.data_name,
           fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.data.data_name_size);
    fdata->value[fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.data
                     .data_name_size] = '\0';
    format_operation_source_prepare(fdata);
    return true;
}
...
static bool format_revoke_sponsorship_data_account(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Account ID", fdata->caption_len);
    FORMATTER_CHECK(print_account_id(
        fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.data.account_id,
        fdata->value,
        fdata->value_len,
        0,
        0))
...
static bool format_revoke_sponsorship_claimable_balance(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Balance ID", fdata->caption_len);
    FORMATTER_CHECK(print_claimable_balance_id(&fdata->envelope->tx.op_details.revoke_sponsorship_op
...
static bool format_revoke_sponsorship_claimable_signer_signer_key_detail(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Signer Key", fdata->caption_len);
    signer_key_t *key = &fdata->envelope->tx.op_details.revoke_sponsorship_op.signer.signer_key;
...
static bool format_revoke_sponsorship_claimable_signer_signer_key_type(formatter_data_t *fdata) {
    ...
    switch (fdata->envelope->tx.op_details.revoke_sponsorship_op.signer.signer_key.type) {
        case SIGNER_KEY_TYPE_ED25519: {
...
static bool format_revoke_sponsorship_claimable_signer_account(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Account ID", fdata->caption_len);
    FORMATTER_CHECK(
        print_account_id(fdata->envelope->tx.op_details.revoke_sponsorship_op.signer.account_id,
                         fdata->value,
                         fdata->value_len,
                         0,
                         0))
...
static bool format_revoke_sponsorship(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Operation Type", fdata->caption_len);
    if (fdata->envelope->tx.op_details.revoke_sponsorship_op.type == REVOKE_SPONSORSHIP_SIGNER) {
        ...
        switch (fdata->envelope->tx.op_details.revoke_sponsorship_op.ledger_key.type) {
            case ACCOUNT:
...
static bool format_clawback_from(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_muxed_account(&fdata->envelope->tx.op_details.clawback_op.from,
                                        fdata->value,
                                        fdata->value_len,
                                        0,
                                        0))
...
static bool format_clawback_amount(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.op_details.clawback_op.amount,
                                 &fdata->envelope->tx.op_details.clawback_op.asset,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_clawback_claimable_balance_balance_id(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Balance ID", fdata->caption_len);
    FORMATTER_CHECK(print_claimable_balance_id(
        &fdata->envelope->tx.op_details.clawback_claimable_balance_op.balance_id,
        fdata->value,
        fdata->value_len,
        0,
        0))
...
static bool format_set_trust_line_set_flags(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Set Flags", fdata->caption_len);
    if (fdata->envelope->tx.op_details.set_trust_line_flags_op.set_flags) {
        FORMATTER_CHECK(
            print_trust_line_flags(fdata->envelope->tx.op_details.set_trust_line_flags_op.set_flags,
                                   fdata->value,
                                   fdata->value_len))
    } else {
...
static bool format_set_trust_line_clear_flags(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Clear Flags", fdata->caption_len);
    if (fdata->envelope->tx.op_details.set_trust_line_flags_op.clear_flags) {
        FORMATTER_CHECK(print_trust_line_flags(
            fdata->envelope->tx.op_details.set_trust_line_flags_op.clear_flags,
            fdata->value,
            fdata->value_len))
    } else {
        STRLCPY(fdata->value, "[none]", fdata->value_len);
...
static bool format_set_trust_line_asset(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Asset", fdata->caption_len);
    FORMATTER_CHECK(print_asset(&fdata->envelope->tx.op_details.set_trust_line_flags_op.asset,
                                fdata->envelope->network,
                                fdata->value,
                                fdata->value_len))
...
static bool format_set_trust_line_trustor(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Trustor", fdata->caption_len);
    FORMATTER_CHECK(print_account_id(fdata->envelope->tx.op_details.set_trust_line_flags_op.trustor,
                                     fdata->value,
                                     fdata->value_len,
                                     0,
                                     0))
...
static bool format_liquidity_pool_deposit_max_price(formatter_data_t *fdata) {
    ...
    uint64_t price =
        ((uint64_t) fdata->envelope->tx.op_details.liquidity_pool_deposit_op.max_price.n *
         10000000) /
        fdata->envelope->tx.op_details.liquidity_pool_deposit_op.max_price.d;
    FORMATTER_CHECK(
        print_amount(price, NULL, fdata->envelope->network, fdata->value, fdata->value_len))
    format_operation_source_prepare(fdata);
    return true;
...
static bool format_liquidity_pool_deposit_min_price(formatter_data_t *fdata) {
    ...
    uint64_t price =
        ((uint64_t) fdata->envelope->tx.op_details.liquidity_pool_deposit_op.min_price.n *
         10000000) /
        fdata->envelope->tx.op_details.liquidity_pool_deposit_op.min_price.d;
    FORMATTER_CHECK(
        print_amount(price, NULL, fdata->envelope->network, fdata->value, fdata->value_len))
...
static bool format_liquidity_pool_deposit_max_amount_b(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.liquidity_pool_deposit_op.max_amount_b,
                     NULL,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
...
static bool format_liquidity_pool_deposit_max_amount_a(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.liquidity_pool_deposit_op.max_amount_a,
                     NULL,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
...
static bool format_liquidity_pool_deposit_liquidity_pool_id(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Liquidity Pool ID", fdata->caption_len);
    FORMATTER_CHECK(
        print_binary(fdata->envelope->tx.op_details.liquidity_pool_deposit_op.liquidity_pool_id,
                     LIQUIDITY_POOL_ID_SIZE,
                     fdata->value,
                     fdata->value_len,
                     0,
...
static bool format_liquidity_pool_withdraw_min_amount_b(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.liquidity_pool_withdraw_op.min_amount_b,
                     NULL,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
...
static bool format_liquidity_pool_withdraw_min_amount_a(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(
        print_amount(fdata->envelope->tx.op_details.liquidity_pool_withdraw_op.min_amount_a,
                     NULL,
                     fdata->envelope->network,
                     fdata->value,
                     fdata->value_len))
...
static bool format_liquidity_pool_withdraw_amount(formatter_data_t *fdata) {
    ...
    FORMATTER_CHECK(print_amount(fdata->envelope->tx.op_details.liquidity_pool_withdraw_op.amount,
                                 NULL,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_liquidity_pool_withdraw_liquidity_pool_id(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Liquidity Pool ID", fdata->caption_len);
    FORMATTER_CHECK(
        print_binary(fdata->envelope->tx.op_details.liquidity_pool_withdraw_op.liquidity_pool_id,
                     LIQUIDITY_POOL_ID_SIZE,
                     fdata->value,
                     fdata->value_len,
                     0,
...
static bool format_confirm_operation(formatter_data_t *fdata) {
    if (fdata->envelope->tx.operations_count > 1) {
        ...
        FORMATTER_CHECK(print_uint(fdata->envelope->tx.operation_index + 1,
                                   op_caption + length,
                                   OPERATION_CAPTION_MAX_LENGTH - length))
        STRLCAT(op_caption, " of ", sizeof(op_caption));
        length = strlen(op_caption);
        FORMATTER_CHECK(print_uint(fdata->envelope->tx.operations_count,
                                   op_caption + length,
                                   OPERATION_CAPTION_MAX_LENGTH - length))
...
static bool format_fee_bump_transaction_fee(formatter_data_t *fdata) {
    STRLCPY(fdata->caption, "Max Fee", fdata->caption_len);
    asset_t asset = {.type = ASSET_TYPE_NATIVE};
    FORMATTER_CHECK(print_amount(fdata->envelope->fee_bump_tx.fee,
                                 &asset,
                                 fdata->envelope->network,
                                 fdata->value,
                                 fdata->value_len))
...
static bool format_fee_bump_transaction_source(formatter_data_t *fdata) {
    ...
    if (fdata->envelope->type == ENVELOPE_TYPE_TX_FEE_BUMP &&
        fdata->envelope->fee_bump_tx.fee_source.type == KEY_TYPE_ED25519 &&
        memcmp(fdata->envelope->fee_bump_tx.fee_source.ed25519,
               fdata->signing_key,
               RAW_ED25519_PUBLIC_KEY_SIZE) == 0) {
        FORMATTER_CHECK(print_muxed_account(&fdata->envelope->fee_bump_tx.fee_source,
                                            fdata->value,
                                            fdata->value_len,
                                            6,
                                            6))
...
static uint8_t get_data_count(formatter_data_t *fdata) {
    uint8_t op_cnt = fdata->envelope->tx.operations_count;
    return op_cnt + 1;
}
