...
uint32_t set_result_get_publicKey() {
    ...
    os_memmove(G_io_apdu_buffer, tmpCtx.publicKeyContext.publicKey, 56);
    tx += 56;
...
void handleGetPublicKey(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
    ...
    os_perso_derive_node_bip32(curve, bip32Path, bip32PathLength, privateKeyData,
                               (tmpCtx.publicKeyContext.getChaincode
                                    ? tmpCtx.publicKeyContext.chainCode
                                    : NULL));
    cx_ecfp_init_private_key(curve, privateKeyData, 56, &privateKey);
