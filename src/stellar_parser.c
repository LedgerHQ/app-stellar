...
static const uint8_t NETWORK_ID_PUBLIC_HASH[64] = {
    0x7a, 0xc3, 0x39, 0x97, 0x54, 0x4e, 0x31, 0x75, 0xd2, 0x66, 0xbd,
...
static const uint8_t NETWORK_ID_TEST_HASH[64] = {
    0xce, 0xe0, 0x30, 0x2d, 0x59, 0x84, 0x4d, 0x32, 0xbd, 0xca, 0x91,
...
bool parse_account_id(buffer_t *buffer, const uint8_t **account_id) {
  ...
  if (!buffer_read32(buffer, &accountType) ||
      accountType != PUBLIC_KEY_TYPE_ED25519) {
    return false;
  }
...
/* TODO: max_length does not include terminal null character */
static bool parse_string_ptr(buffer_t *buffer, const char **string,
                             size_t *out_len, size_t max_length) {
  uint32_t size;
...
static bool parse_create_account(buffer_t *buffer,
...
static bool parse_path_payment(buffer_t *buffer,
...
static bool parse_account_merge(buffer_t *buffer,
...
static bool parse_manage_data(buffer_t *buffer, manage_data_op_t *manageData) {
  ...
  if (!parse_string_ptr(buffer, (const char **)&manageData->dataName, &size,
                        DATA_NAME_MAX_SIZE)) {
    return false;
  }
  ...
  if (hasValue) {
    if (!parse_string_ptr(buffer, (const char **)&manageData->dataValue, &size,
                          DATA_VALUE_MAX_SIZE)) {
      return false;
    }
...
static bool parse_active_offer(buffer_t *buffer,
...
static bool parse_change_trust(buffer_t *buffer,
...
static bool parse_optional_type(buffer_t *buffer, xdr_type_parser parser,
                                void *dst, bool *opted) {
  bool isPresent;
...
static bool parse_set_options(buffer_t *buffer, set_options_op_t *setOptions) {
  ...
  if (!parse_optional_type(buffer, (xdr_type_parser)parse_account_id,
                           &setOptions->inflationDestination,
                           &setOptions->inflationDestinationPresent)) {
    return false;
  }

  if (!parse_optional_type(buffer, (xdr_type_parser)buffer_read32,
                           &setOptions->clearFlags, NULL)) {
    return false;
  }
  if (!parse_optional_type(buffer, (xdr_type_parser)buffer_read32,
                           &setOptions->setFlags, NULL)) {
    return false;
  }
  if (!parse_optional_type(buffer, (xdr_type_parser)buffer_read32,
  ...
  if (homeDomainPresent) {
    ...
    if (!check_padding(
        setOptions->homeDomain, setOptions->homeDomainSize,
  ...
  return parse_optional_type(buffer, (xdr_type_parser)parse_signer,
                             &setOptions->signer, &setOptions->signerPresent);
}

static bool parse_bump_sequence(buffer_t *buffer,
...
static bool parse_op_xdr(buffer_t *buffer, operation_details_t *opDetails) {
  ...
  if (!parse_optional_type(buffer, (xdr_type_parser)parse_account_id,
                           &opDetails->source, &opDetails->sourcePresent)) {
    return false;
  }
...
bool parse_tx_xdr(const uint8_t *data, size_t size, tx_context_t *txCtx) {
  ...
  if (offset == 0) {
    ...
    if (!parse_optional_type(&buffer, (xdr_type_parser)parse_time_bounds,
