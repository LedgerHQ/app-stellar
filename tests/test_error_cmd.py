...
def test_wrong_p1p2(backend):
    with pytest.raises(ExceptionRAPDU) as e:
        backend.exchange(cla=CLA, ins=InsType.GET_CONF, p1=P1.FIRST_APDU + 1, p2=P2.MORE_APDU)
    assert e.value.status == Errors.SW_WRONG_P1_P2
    with pytest.raises(ExceptionRAPDU) as e:
        backend.exchange(cla=CLA, ins=InsType.GET_CONF, p1=P1.FIRST_APDU, p2=P2.MORE_APDU)
    assert e.value.status == Errors.SW_WRONG_P1_P2
